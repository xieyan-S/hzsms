...
(function($) {
  ...
  $(function() {
    ...
    //$("select[name='place']").selectpicker({style: 'btn-primary', menuStyle: 'dropdown-inverse'});
    $("select").selectpicker({style: 'btn-primary'});
    //$("select[id='place']").selectpicker({style: 'btn-primary'});
    $("select[id='place']").change(function(){
      if($(this).val() == -1){
        $("#customplace").removeClass("hide");
      }else{
        if(!$("#customplace").hasClass("hide")){
          $("#customplace").addClass("hide");
        }
      }
    });
